...
- **arm64가 최우선 제약**이다. 이미지·베이스 이미지(`v1.61.0-noble` = Ubuntu 24.04)·CI 러너(`ubuntu-26.04-arm`)를 전부 arm64로 맞춘다. amd64를 배포하면 서버에서 `exec format error`가 난다.
...
- **`UV_SYSTEM_PYTHON=1` 금지**: uv 관리 Python 3.14.6(pin)을 쓴다. noble(24.04) 이미지 내장 시스템 Python은 **3.12**라, 시스템 Python을 강제하면 `requires-python = ">=3.14"`와 충돌하거나 uv.lock 재현성이 깨진다.
...
- **파이썬이 3.12로 잡힘/uv.lock과 어긋남**: noble 내장 시스템 Python은 3.12다. `uv python install 3.14 && uv sync --python 3.14` 후 `ENV PATH="/app/.venv/bin:$PATH"`가 빠졌거나 `UV_SYSTEM_PYTHON=1`이 걸리면 uv가 pin한 3.14.6 대신 시스템 3.12가 잡힌다.
...
- **인프라 저장소(외부)**: Alloy scrape 타깃 추가(`worker-<워커키>:9464` — 워커별 정적 등록), `crawl_parse_fallback_total` 알림 추가, `dead:<agent>` check-keys 추가 등 인프라 측 필요 사항은 직접 변경하지 않고 보고서의 "인프라 저장소 요청 사항"으로 정리해 전달한다.
