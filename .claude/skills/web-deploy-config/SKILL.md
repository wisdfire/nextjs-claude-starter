...
Next.js 웹앱의 **배포 설정을 코드로 관리(config-as-code)** 하고 Vercel에 배포하는 규약이다. 이 저장소에서 인프라를 다루는 **유일한 방법**이다.

## 왜 config-as-code인가 (이 저장소의 IaC)

이 스택의 인프라(빌드·CDN·서버리스 함수·크론)는 **Vercel이 관리**한다. 그래서 서버를 직접 프로비저닝하는 대신, 프로젝트 설정 자체를 **버전관리되는 코드(`vercel.ts`)** 로 둔다. 이유:
...
EC2 등 **자체 인프라에 직접 배포**하는 경우(크롤링 워커·Python 배치 등)는 이 저장소의 대상이 아니다. 그 인프라와 배포는 별도 모노레포 **`wisdfire/jobhub-jobs`** 가 소유한다. 여기서 서버 프로비저닝을 정의하지 않는다.
...
- 셀프호스팅 인프라(크롤링 워커·Python 배치 서버)를 여기서 다루지 않는다 — `wisdfire/jobhub-jobs` 모노레포 소관.
