...
⚠️ **DB에서 온 텍스트를 LLM으로 번역해 쌓지 않는다.** 구글 검색 스팸 정책은 "**자동 변환(동의어 치환·번역 등)으로 페이지를 대량 생성해 사용자 가치를 거의 주지 않는 것**"을 scaled content abuse로, Publisher Policies는 "**수동 검토·큐레이션 없는 자동 생성 콘텐츠**"를 각각 금지한다. (⚠️ "machine-translated text without human review"는 **현행 문서에 없는 문구**이니 구글 인용으로 쓰지 말 것 — 근거는 `adsense-readiness` MUST 6) 열거값(카테고리·상태)은 **코드에 키로 정의해** 번역하고, DB에는 그 키만 저장한다.
