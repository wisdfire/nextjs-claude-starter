...
| 팀원 | 에이전트 타입 | 역할 | 스킬 | 출력 |
|------|--------------|------|------|------|
| design-architect | general-purpose (`.claude/agents/design-architect.md`) | 와이어프레임·IA·라우트 매핑·디자인 시스템·컴포넌트 명세 | `wireframe-design`, `ui-ux-pro-max`, `frontend-design` | `_workspace/01_design_spec.md` |
| frontend-engineer | general-purpose (`.claude/agents/frontend-engineer.md`) | Next.js 16 화면·컴포넌트·훅 구현 + Vitest 테스트 | `frontend-build`, `ui-ux-pro-max` | 코드 + 테스트 + `_workspace/03_frontend_notes.md` |
| backend-engineer | general-purpose (`.claude/agents/backend-engineer.md`) | API route·Supabase·Drizzle·서버 액션 + Vitest 테스트(shape 고정) | `backend-api` | 코드 + 테스트 + `_workspace/04_api_contract.md` |
| qa-inspector | general-purpose (`.claude/agents/qa-inspector.md`) — Explore 금지(스크립트 실행 필요) | 경계면 교차 검증(링크·shape·매핑) + `npm run test` 게이트 | `qa-link-integrity` | `_workspace/05_qa_report.md` |
...
2. 작업 등록(`depends_on`으로 파이프라인 의존성 명시):
   ...
   TaskCreate(tasks: [
     ...
     { title: "점진 QA(백엔드)", description: "API 완성 즉시 shape↔훅·엔드포인트 매핑 검증", assignee: "qa-inspector", depends_on: ["백엔드 구현"] },
...
2. **명령 게이트 순서 통과**: 리더가 `npm run lint` → `npm run build` → `npm run test`(Vitest) 순으로 실행해 전부 통과시킨다. 하나라도 실패하면 해당 팀원에 파일:라인과 함께 통보하고 수정 → 재검증 루프로 돌린다.
...
- [ ] **`vercel.ts` 설정 코드화** — 배포 설정이 `@vercel/config`로 코드화(`vercel.json` 수기 작성 없음), 환경변수 스코프 구분
- [ ] 화면 흐름 변경 시 Playwright MCP 브라우저 검증

### Phase 4.5: 배포 (config-as-code)

검증 게이트를 통과한 뒤 리더가 `web-deploy-config` 스킬을 로드해 Vercel에 배포한다.
