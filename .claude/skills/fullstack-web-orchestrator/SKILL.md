...
description: "풀스택 웹사이트 개발 에이전트 팀을 조율하는 오케스트레이터. 웹사이트 개발, 화면 구현, 풀스택 기능 개발, 와이어프레임→디자인→프론트→백엔드→QA 파이프라인 실행 시 사용. 후속 작업: 결과 수정, 보완, 다시 실행, 재실행, 부분 재구현, 이전 결과 개선, QA, 링크 검증, 라우팅 점검, API 계약 갱신, 테스트, 배포, deploy, vercel, 검증 게이트 요청 시에도 반드시 이 스킬을 사용. **층별 순차 구동(scoped)**: '화면그룹/층 X만 구현하고 정지', 'impl-progress.md 기준 다음 층', 큰 ROADMAP을 층 단위로 나눠 호출하는 요청에도 이 스킬을 사용."
---
...
> **🚨 애드센스 심사 통과는 타협 불가 목표다(최우선).** 이 하네스로 만드는 모든 웹서비스는 구글 애드센스 심사를 통과해야 한다. **리더와 모든 팀원은 `adsense-readiness` 스킬을 로드**하고 그 **공식 MUST 10항**을 설계·구현·QA 전 단계에서 충족시킨다. 애드센스 MUST 미충족은 **Phase 4.3 게이트에서 배포·완료를 차단**하고, 기계 검증으로 판정되지 않는 정성 축(저가치·복제·편집 부가가치)은 **Phase 4.4의 적대적 검증(`adsense-adversarial-audit`, 최대 3라운드)**이 차단한다. 나중에 고치는 것이 아니라 **처음부터 요건을 만족하도록 설계**한다(정책 페이지 라우트·광고 허용 화이트리스트·콘텐츠 오리지널리티·**편집 뎁스 4종 레이어**). ⚠️ **"편집 부가가치"를 템플릿 한 줄로 끝내지 마라** — 파생 산출물이 存在해도 페이지마다 숫자만 바뀐 동일 문장이면 저가치로 읽힌다(streamprice 학습: 적대적 감사 통과 후 뎁스 부족으로 배포 후 보완 필요). 뎁스는 설계·구현의 양의 요구사항이며 정의는 `adsense-readiness` §편집 뎁스.
>
...
2. 모드 결정:
   ...
   - **층 범위 지정 요청**(예: "화면그룹 `<X>`만 구현하고 정지", "층 `<N>`만", `_workspace/impl-progress.md` 존재) → **층별 순차 구동(scoped construction)**. 큰 ROADMAP을 상위 오케스트레이터가 층 단위로 나눠 이 하네스를 **N회 호출**하는 모드다. **아래 "Phase 2-S"로 진행**한다(Phase 1의 화면설계서 인계 확인은 동일 수행). 한 팀원이 전 화면을 한 세션에 이고 가는 것을 막아 캐시 재읽기 폭발(P1)을 끊는 것이 목적이다.
   ...
   > **부분 재실행 vs 층별 순차 구동은 다르다.** 전자는 *완성된 산출물의 수정·보완*(touch-up)이고, 후자는 *처음부터 층 단위로 나눠 짓는 신규 건설*이다. 층별 구동은 이전 층의 **커밋된 코드**를 baseline으로 읽되 그 층을 "고치는" 게 아니라 **다음 층을 새로 짓는다**.
     ...
     members: [
       { name: "design-architect",  agent_type: "design-architect",  model: "opus",
         prompt: /* 화면설계서 모드 */ "_workspace/00_input/02_screen-spec.md·01_design-system.md·00_brand-guideline.md를 **먼저 Read**하라. 이 문서들이 화면·컴포넌트·토큰·문구의 SSOT다. 당신의 일은 **새로 설계하는 것이 아니라 수용·번역·검증**이다: ①화면설계서의 화면 목록을 그대로 받아 Next.js 16 App Router 라우트로 매핑한다(route group·동적 세그먼트 정확히 — 이 표가 QA의 SSOT다) ②컴포넌트를 shadcn 카탈로그에 대응시킨다 ③01_design-system.md의 토큰을 shadcn 토큰 체계로 매핑한다(신규 정의 금지, globals.css의 :root/.dark/@theme inline 수정 금지) ④화면설계서와 애드센스 MUST(adsense-readiness 스킬)가 충돌하면 — 예: 정책 페이지 라우트 누락, 저가치 화면 광고 — 임의로 고치지 말고 리더에게 SendMessage로 보고한다. **화면을 추가·삭제·재설계하지 않고, 팔레트·폰트·무드를 재결정하지 않는다**(이미 확정된 브랜드다). 결과를 _workspace/01_design_spec.md에 쓴다."
                 /* 폴백(docs/design/ 부재) */ "화면설계서가 없다 — 자체 설계 모드다. wireframe-design 스킬로 _workspace/00_input의 요구사항을 IA·라우트 매핑·와이어프레임·디자인 토큰 방침·컴포넌트 명세로 설계해 _workspace/01_design_spec.md에 쓴다. 시각 방향은 ui-ux-pro-max·frontend-design 스킬로 결정한다. 라우트 표는 QA의 SSOT이니 정확히. **편집 뎁스(adsense-readiness §편집 뎁스 4종 레이어)를 화면 목록·라우트에 반영**한다 — 데이터 화면만 나열하지 말고 에버그린 가이드·방법론·FAQ 라우트를 넣고, 데이터 화면 해설은 데이터 조건별 분기 상태를 명세한다(템플릿 한 줄 금지)." },
       { name: "frontend-engineer", agent_type: "frontend-engineer", model: "opus",
         prompt: "frontend-build 스킬로 01_design_spec.md의 라우트 표를 따라 app/ 화면·컴포넌트·훅을 구현. **화면설계서(_workspace/00_input/02_screen-spec.md·01_design-system.md)가 있으면 그것도 직접 Read하고, 문구·컴포넌트·토큰은 거기 있는 것을 쓴다(임의 생성 금지)** — 01_design_spec.md는 라우트 매핑 계층일 뿐 화면 정의의 원본이 아니다. shadcn은 npx로 추가, base-ui render prop 사용. 훅 타입은 04_api_contract.md에 맞춘다. **편집 뎁스(adsense-readiness §편집 뎁스 4종 레이어) 구현 필수**: 데이터 화면의 해설 문단은 데이터 조건(신규/인상/동률/결측)에 따라 분기해 렌더하고 '숫자만 바뀐 한 문장'으로 끝내지 않는다(전 페이지 동형 = 대량생성 리스크). 설계에 에버그린 가이드·방법론·FAQ 화면이 있으면 데이터 화면과 함께 구현한다. **다국어 필수(i18n-localization 스킬 로드)**: 페이지는 app/[locale]/ 아래에 만들고 setRequestLocale 호출, 사용자 노출 문자열은 전부 messages/ko.json·messages/en.json에 두고 useTranslations로 읽으며(aria-label·placeholder·토스트·메타데이터 포함), 링크·이동은 @/i18n/navigation의 Link·useRouter·redirect를 쓴다(next/link 금지). 키는 두 언어에 동시 추가. 컴포넌트·훅에 Vitest 테스트(@testing-library/react·renderHook)를 함께 작성해 npm run test 통과." },
...
### Phase 2-S: 층별 순차 구동 (scoped 모드 — 큰 ROADMAP의 P1 토큰 조치)

> Phase 0에서 **층별 순차 구동**으로 분기했을 때만 이 절을 따른다. 초기/부분/새 실행은 위 Phase 2를 쓴다.

**왜**: 한 `frontend-engineer`가 전 화면(수십~수백 파일)을 한 세션에 이고 가면 매 턴 전체를 재읽기해 캐시 읽기가 **준-2차로 폭발**한다(실측: 단독 573M = 한 구현 세션 전체 캐시읽기의 56%, 전체의 96%). 상위 오케스트레이터가 층 단위로 이 하네스를 **N회 호출**하고, 각 호출은 **새 세션이라 컨텍스트가 리셋**된다. 이 절은 그 각 호출이 **지정 층만** 하고 멈추게 한다.

1. **baseline 회수(정지 조건 포함)**: `_workspace/impl-progress.md`(있으면) + 커밋 로그로 **어느 층까지 done인지** 확인한다. 지정 층의 **선행 층이 done이 아니면 정지·보고**한다(순서를 건너뛰면 의존이 깨진다).
2. **범위 한정 태스크만 등록**: Phase 2의 `TaskCreate`에서 **이번 층에 해당하는 것만** 넣는다:
   - 지정 화면그룹의 backend(필요 시)·frontend·**점진 QA(그 층)**.
   - **통합 QA(Phase 4)·AdSense 게이트(4.3/4.4)·배포는 등록하지 않는다** — 마지막 층 호출에서만 수행한다.
   - `frontend-engineer` 프롬프트에 **스코프된 읽기**를 명시한다: *"이 층의 `02_screen-spec.md` 해당 절 + 이 층이 새로 만들거나 수정하는 파일만 Read하라. 이미 done인 층의 코드는 `impl-progress.md`와 커밋으로만 확인하고 **통째로 다시 읽지 마라**."* — 이 지시가 빠지면 팀원이 전 코드를 재읽기해 절감이 사라진다(조치의 핵심).
3. **커밋 + 인계물 갱신**: 층 완료 후 커밋하고 `_workspace/impl-progress.md`에 한 줄 추가한다: `| 층 | done | <커밋 SHA> | <시각 KST> | <다음 층> |`.
4. **정지**: 지정 층만 하고 종료한다. 다음 층은 상위 오케스트레이터가 **새 호출**로 시작한다(그래야 컨텍스트가 리셋된다).
5. **마지막 층**: 상위가 "마지막 층 + 통합 검증까지"로 호출하면, 그 층 구현 후 **Phase 4(통합 QA)·4.3/4.4(AdSense 게이트)**를 정상 수행하고 `_workspace/05_verification.md`를 낸다(완료 판정 산출물).

**층 경계 기준**: `docs/design/02_screen-spec.md`·`docs/ROADMAP.md`의 **의존성 순서**(스키마→읽기계층→공통(레이아웃·라우팅·정책화면)→데이터화면 그룹→AdSense/통합QA). **3~6층으로 적정 분할** — 너무 잘게 쪼개면 층마다 공통 맥락 재읽기 세금이 절감을 잡아먹는다.

> **작은 ROADMAP은 이 모드를 쓰지 않는다.** 한 세션 예산(5시간)에 들어갈 규모면 위 Phase 2 단발 완주가 낫다 — 층 분할의 이음새·재오리엔테이션 비용을 피한다. 이 모드는 **큰 D 전용**이다.
...
> **`_workspace/impl-progress.md` (층별 순차 구동 모드 전용 · 리더가 유지)**: 층별 구동에서 리더가 각 층 완료 시 한 줄씩 append하는 **인계 원장**이다. 스키마: `| 층 | 상태(done/wip) | 커밋 SHA | 완료시각(KST) | 다음 층 |`. 다음 층 호출(새 세션)이 이 파일 + 커밋 코드를 baseline으로 읽어 **어디까지 됐는지**를 요약이 아니라 사실로 안다. 단발/부분 모드에서는 만들지 않는다.
...
3. **3렌즈 독립 병렬 심사**: `adsense-auditor` 3개를 **서로의 결론을 모른 채** 동시에 띄운다(앵커링 방지). 팀 모드면 팀원으로, 아니면 `Agent`(`model: "opus"`, `run_in_background: true`)로 실행한다.
   ...
   | 렌즈 | 심사 축 | 출력 |
   |---|---|---|
   | **L1 콘텐츠 가치** | 복제·저가치·대량생성·빈 상태·미완성·본문 기계번역 + **얇음/템플릿 파생(formulaic)·편집 뎁스 4종 유형 커버리지** (MUST 5·6·7·10) | `_workspace/06_adsense_audit_r{N}_L1.md` |
...
4. **CLAUDE.md 현행화 (구현이 완결된 호출에서만)** — 템플릿에서 태어난 저장소의 CLAUDE.md는 정체성이 제네릭(`Next.js Starter Kit`)이고 `## 이 스타터킷의 사용 흐름` 절이 **빈 스타터킷 전제**다. 구현을 마쳤으면 다음 세션이 이 저장소를 열었을 때 **실제 구축 상태**를 보도록 현행화한다. **Phase 4.3 통합 검증을 수행한 호출에서만** 한다(= 단발 완주 또는 층별 순차 구동의 **마지막 층**. 중간 층 호출은 Phase 4를 돌리지 않으므로 자연히 건너뛴다).
   - **정체성 헤더**(`**프로젝트**:` 줄): 아직 제네릭(`Next.js Starter Kit …`)이면 제품명·한 줄 요약으로 교체. **이미 제품명으로 바뀌어 있으면 건드리지 않는다**(멱등 — 파이프라인 안에서는 상위 오케스트레이터가 정체성·경계를 이미 채웠을 수 있다).
   - **`## 이 스타터킷의 사용 흐름` → `## 현재 프로젝트 상태 (<제품명>)` 로 교체**(빈 스타터킷 절을 실제 상태로 대체):
     - **구현 범위**: 화면 수·`app/[locale]/` 라우트 목록·랜딩 위치(루트 `app/page.tsx` 유무).
     - **데이터 모델**: `lib/db/schema.ts`의 테이블, 읽기 쿼리 위치(`lib/db/queries/`), 픽스처 위치(`lib/db/fixtures.ts`).
     - **gotcha(반드시 알아야 할 비자명 가정)**: 구현 중 내린 결정 중 **다음 세션이 모르면 깨지는 것만** 적는다 — 예) 시드 없으면 데이터 페이지 `notFound()`, 광고 env 게이팅, 테마 기본값, 로고/이미지 폴백. **일반 규칙(모든 프로젝트에 참인 것)은 쓰지 않는다.** 이 프로젝트에서 실제로 성립하는 함정만.
   - **명령어**: 이 프로젝트가 새로 도입한 스크립트(예: `db:seed`)를 명령어 블록에 추가.
   - **변경 이력** 표에 현행화 한 줄 추가.
   - ⚠️ **본문 규칙(`## 핵심 규칙`·`## 문서 색인`·`## 하네스`)은 손대지 않는다** — 얇은 라우터라 어느 프로젝트에나 유효하다. 현행화는 **정체성·경계·현재 상태·명령어**에 국한한다(그 외를 고치면 라우터가 프로젝트별로 갈라진다).
   - 커밋: `docs: CLAUDE.md 현재 프로젝트 상태 현행화 (<제품명>)`.
5. 커밋 전 검증은 Phase 4.3 검증 게이트(`npm run lint`→`build`→`test`)와 Phase 4.5 배포가 이미 통과·완료되었는지 리더가 확인한다. 화면 흐름 변경 시 Playwright MCP로 브라우저 검증.
6. 사용자에게 결과를 요약 보고한다(생성/수정 화면, API, QA 결과, Vitest 테스트 통과, **애드센스 적대적 검증 최종 판정 + 라운드 수 + 잔여 리스크**, `vercel.ts` 설정 코드화·배포 결과 요약, **CLAUDE.md 현재 프로젝트 상태 현행화 여부**).
