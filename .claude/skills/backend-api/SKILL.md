---
name: backend-api
description: "API route·서버 액션·Drizzle 스키마를 구현할 때 사용한다. 데이터 모델링, 엔드포인트 작성, 마이그레이션, 그리고 응답 shape을 프론트 훅과 계약으로 관리해야 하는 백엔드 구현 순간에 반드시 켠다."
---

# Backend API — Route + Drizzle
...
```ts
import { db } from "@/lib/db";   // 서버 전용. 이게 유일한 경로다.
```

- **모든 DB 접근은 서버 사이드 Drizzle 단일 경로**다. 브라우저가 DB에 직접 붙는 경로는 **없다**.
- **`lib/db`를 클라이언트 컴포넌트에서 import 하지 않는다** — 번들로 새는 순간 전제가 깨진다.
- **DB는 외부에 노출되지 않으므로 RLS 같은 행 수준 방어가 없다.**
  ⚠️ 뒤집어 말하면 **"무엇을 공개할지"를 걸러 줄 계층이 DB에 없다** — 공개 조건(예: `published = true`)은
  **쿼리의 `where` 절이 책임진다.** 그 필터를 빠뜨리면 미공개 데이터가 그대로 나간다.
- `lib/db`는 `DATABASE_URL` 부재 시 **즉시 throw**한다(조용히 빈 결과를 주지 않는다) — 우회하지 말 것.

> 인증이 필요해지면 그때 별도로 설계한다. 미리 깔지 않는다 — 인증 없는 사이트에 인증 계층을 두면
> 매 요청마다 아무것도 안 하는 왕복만 늘어난다.
...
- **DB 계층은 모킹**한다: 실제 DB 없이 로직만 검증하기 위해 `vi.mock("@/lib/db")`로 데이터 접근 함수를 스텁한다. 네트워크·DB에 의존하지 않게 한다.
  > **쿼리 빌더 체이너를 손으로 흉내 내지 마라.** `db.select().from().where()` 체인을 목으로 재현하면 목이 깨지기 쉽고, 무엇보다 **타입이 맞는지 아무것도 증명하지 못한다.** 데이터 접근을 `lib/db/queries/*.ts`의 **함수**로 감싸고 그 함수를 모킹하는 편이 훨씬 견고하다.
- **응답 shape 계약 고정 테스트**: 핸들러(또는 서버 액션)를 호출해 반환된 응답 본문의 **키·래핑·필드 케이스**가 계약과 정확히 일치하는지 단언한다.
  ```ts
  import { vi, describe, it, expect } from "vitest";

  // 데이터 접근 함수를 모킹해 DB 없이 핸들러 로직만 검증
...
- Next.js/Drizzle API를 추측으로 쓰지 않는다 — 문서/Context7로 확인.
- 비밀·키(`DATABASE_URL`·`DIRECT_URL`)를 코드에 하드코딩하지 않는다 — 환경 변수로만. **연결 문자열 자체가 비밀값이다**(비밀번호 포함) — 로그에도 남기지 않는다. `lib/db`의 `db`가 클라이언트로 새어나가게 하지 않는다.
...
- 테스트에서 `.only`/`.skip`으로 실패를 덮지 않고, 모킹은 DB·네트워크 등 외부 경계에만 하며(검증 대상 핸들러 로직 자체는 실제 실행), 실패 시 기대값만 바꿔 통과시키지 않는다.

## 애드센스 요건 (서버 계층 필수)

> **`adsense-readiness` 스킬이 단일 진실 공급원**이다. 반드시 함께 로드하라. 서버 계층도 애드센스 심사 결과를 좌우한다.

- **수집 데이터를 원본 그대로 응답에 담지 않는다(최대 리스크)** — 모노레포 `wisdfire/jobhub-jobs`가 DB에 적재한 데이터를 그대로 전달하면 프론트가 그대로 렌더링해 **복제·저가치 콘텐츠로 거절**된다. 집계·정규화·요약·비교 등 **편집적 부가가치를 서버에서 만들어** 응답 shape에 담고 `04_api_contract.md`에 명시한다.
- **공개 콘텐츠는 서버에서 읽어 ISR로 내보낸다** — 심사 대상 페이지가 빈 화면이 되면 "No content"로 거절된다. DB는 외부 비노출이라 익명 읽기 정책 같은 것은 없다 — 서버 컴포넌트가 `lib/db`로 읽어 정적 생성/ISR로 제공하는 것이 공개 경로다.
- **빈 응답이 "준비 중" 플레이스홀더 화면으로 이어지지 않게** 빈 상태를 계약에 정의하고 프론트에 통지한다(플레이스홀더 = 대표 거절 사유).
- `robots`·`sitemap`을 route handler로 구현할 때 **`Mediapartners-Google`을 차단하지 않는다**(전역 `Disallow: /` 금지).
- ⚠️ **크롤러·Python 배치·ETL은 이 저장소에서 구현하지 않는다** — 모노레포 `wisdfire/jobhub-jobs` 소관. 여기서는 적재된 데이터를 **읽어서 가공해 제공**하는 것까지 담당한다.
