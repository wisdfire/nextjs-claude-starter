...
- **수집 데이터를 원본 그대로 응답에 담지 않는다(최대 리스크)** — 모노레포 `wisdfire/jobhub-jobs`가 Supabase에 적재한 데이터를 그대로 전달하면 프론트가 그대로 렌더링해 **복제·저가치 콘텐츠로 거절**된다. 집계·정규화·요약·비교 등 **편집적 부가가치를 서버에서 만들어** 응답 shape에 담고 `04_api_contract.md`에 명시한다.
- **공개 콘텐츠는 익명 읽기 가능해야 한다** — RLS·인증 때문에 심사 대상 페이지가 빈 화면이 되면 "No content"로 거절된다.
