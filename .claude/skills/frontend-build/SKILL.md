...
- **Server Component**: `async` 컴포넌트에서 직접 `await`로 데이터를 가져온다(fetch 또는 서버 측 Supabase `createClient()`).
...
- Next.js 16에서 미들웨어 파일은 `middleware.ts`가 아니라 **`proxy.ts`**다. 인증 세션 갱신 등은 `lib/supabase/middleware.ts`의 헬퍼를 `proxy.ts`에서 호출한다.
...
- 클라이언트 노출 금지 값에 `NEXT_PUBLIC_`을 붙이지 않고, 서버 전용(`lib/db`·서버 Supabase 클라이언트)을 클라이언트 컴포넌트로 import하지 않는다.
...
- **수집 데이터를 원본 그대로 렌더링하지 않는다** — 요약·구조화·해설 등 편집 부가가치를 구현한다(복제 콘텐츠 금지). 수집 자체는 모노레포 `wisdfire/jobhub-jobs` 소관이며, 웹앱은 Supabase에 적재된 데이터를 읽어 **가공해서** 보여준다.
