...
| 에이전트 | 담당 | 사용 스킬 | 산출물 |
| --- | --- | --- | --- |
| crawl-worker-engineer | **구현담당(리드 백엔드 엔지니어)**. Celery 워커(기본 큐 `celery` 고정·acks_late·멱등 upsert), Playwright sync + 이미지 내장 Chromium, 하이브리드 파싱(Locator 메인 80% + ScrapeGraphAI 폴백 20%), prometheus_client 계측(`worker_process_init` 1회 바인딩), Celery Beat 스케줄(별도 서비스), 2단 이미지(base→agent) + arm64 빌드 + ECR + SSM 배포, 테스트(fixture·폴백 트리거·계약 스키마) | `celery-crawl-worker`(계약, 필수 선로드), `python-test-ci`, `playwright-scraping`(셀렉터·Anti-Bot 참고), `supabase-upsert-load`(결과 저장=DB 확정 시) | `_workspace/05_crawl_worker.md` + 워커 모노레포 코드 |
...
- **조정 포인트 확인**: 워커키(인프라 `workers` 맵 등록 여부), GitHub Variables 5종(`AWS_ROLE_ARN`·`ECR_REPOSITORY_URL`·`EC2_INSTANCE_ID`·`DEPLOY_DIR`·`SSM_PARAM_PATH`), 첫 에이전트 대상 사이트, 결과 저장 계층(result 큐 소비 vs DB upsert), LLM 공급자, `HEALTHCHECK_URL`이 확정됐는지 사용자 요청에서 확인하고, 미확정 항목은 스텁으로 진행함을 명시한다. ⚠️ 같은 Valkey를 쓰는 **다른 워커 저장소가 이미 가동 중**이면 기본 `celery` 큐 공유로 태스크가 상호 오소비되므로, 큐 분리 방안이 인프라와 확정되기 전에는 실가동(디스패치 활성화)하지 않는다.

### Phase 1: 요구 확정
- 대상 에이전트 그룹(뉴스/커머스/게시판 등), 사이트 목록, 크론 스케줄, 추출 필드(CrawlResult 스키마·필수 필드)를 확정한다.
- **버전은 인프라에서 파생한다.** `celery-crawl-worker` 스킬의 "버전 매트릭스"를 그대로 따른다(인프라 `CLAUDE.md`의 워커 스택 표 + `docs/03_cicd.md` §4-1이 원본). 핵심: 베이스 `mcr.microsoft.com/playwright/python:v1.61.0-noble`(`noble` = Ubuntu 24.04 LTS) · pip `playwright==1.61.0`(태그와 일치) · `uv python install 3.14`로 Python 3.14.6 pin(noble 내장 시스템 Python은 3.12 — uv 설치가 필수) · `UV_SYSTEM_PYTHON=1` 금지 · uv 바이너리 버전 pin · ScrapeGraphAI는 `==2.1.5`(3.x는 Python 3.14 미지원 이슈로 보류).
...
- crawl-worker-engineer를 호출한다(`model: "opus"`). 에이전트는 `celery-crawl-worker` 계약 스킬을 먼저 로드하고:
  ...
  3. **단계 구현** — Celery 앱·CrawlJob 모델·브라우저 수명주기·하이브리드 파싱 프레임·Prometheus 계측(`worker_process_init` 1회 바인딩)·Healthchecks ping·첫 에이전트(스텁)·Beat 스케줄(별도 서비스).
...
### Phase 4: CI · 배포 연결
- 트리거는 `on: { push: { branches: [main] }, workflow_dispatch: {} }`. 모노레포(웹앱+워커)면 **`paths: ["crawl-worker/**", ...]` 필터 필수**(없으면 웹앱 커밋마다 4GB 이미지 배포). PR 트리거는 선택이며, 붙일 경우 테스트 게이트만 돌고 배포하지 않는다.
...
- GitHub **Variables**(Secrets 아님) **5종**: `AWS_ROLE_ARN`, `ECR_REPOSITORY_URL`, `EC2_INSTANCE_ID`, `DEPLOY_DIR`(`/opt/crawling-worker-<워커키>`), `SSM_PARAM_PATH`. 인프라 저장소의 `tofu output worker_deploy_values`로 얻는다. **preflight 잡**으로 5종 존재를 먼저 검사해 미설정 시 빌드·배포를 조건부 스킵한다.
...
- 인프라 저장소 쪽 변경은 이 하네스 범위 밖이다 — 아래를 보고서에 **"인프라 저장소 요청 사항"** 으로 정리해 전달한다(직접 수정 금지):
  1. `config.alloy`의 `crawling-worker` job에 **이 워커의 정적 타깃(`worker-<워커키>:9464`) 한 줄 추가** — 워커별 정적 등록이라 추가해야 `crawling_app` 알림 그룹이 동작한다(compose 컨테이너명도 `worker-<워커키>`로 일치시킬 것).
...
- README에는 서버 최초 1회 준비(`DEPLOY_DIR`=`/opt/crawling-worker-<워커키>/` 배치, `.env` 600 권한, `VALKEY_PASSWORD`는 SSM Parameter Store에서 렌더)와 GitHub Variables 5종 세팅 안내를 포함하고, 미설정 시 ECR push까지만 동작함을 명시한다.
...
- **파이썬이 3.12로 잡힘/uv.lock과 어긋남**: noble 이미지 내장 시스템 Python은 3.12다. `ENV PATH="/app/.venv/bin:$PATH"` 누락 또는 `UV_SYSTEM_PYTHON=1`이 원인으로, uv가 pin한 3.14.6 대신 시스템 3.12가 잡힌다.
...
- [ ] **이미지 정합**: 베이스 = `mcr.microsoft.com/playwright/python:v1.61.0-noble`(Ubuntu 24.04), pip `playwright==1.61.0`(이미지 태그와 일치), `uv python install 3.14` + `uv sync --python 3.14` + `ENV PATH=/app/.venv/bin:$PATH`, `pyproject.toml`의 `requires-python = ">=3.14"`, uv 바이너리는 버전 pin(`:latest` 금지), `UV_SYSTEM_PYTHON=1` 없음
