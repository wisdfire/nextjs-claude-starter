...
크롤링 워커 애플리케이션(MAS)을 구현·확장한다. 실행 인프라(**AWS EC2 t4g.small 단일 노드 · ARM64 Graviton2 · 2 vCPU/2 GiB · Ubuntu LTS · Docker Compose · Valkey 브로커 · ECR · Grafana Alloy→Grafana Cloud**)는 **별도 인프라 저장소 `crawling-node-infra`** 에서 OpenTofu(`opentofu-infra` 스킬 규약)로 구축·운영되며, 이 하네스는 그 위에서 실행될 **워커 에이전트 구현과 arm64 이미지 배포(ECR push → SSM send-command)만** 담당한다.
...
- **버전은 인프라에서 파생한다.** `celery-crawl-worker` 스킬의 "버전 매트릭스"를 그대로 따른다(인프라 `CLAUDE.md`의 워커 스택 표 + `docs/03_cicd.md` §4-1이 원본). 핵심: 베이스 `mcr.microsoft.com/playwright/python:v1.61.0-noble` · pip `playwright==1.61.0`(태그와 일치) · `uv python install 3.14`로 Python 3.14 · `UV_SYSTEM_PYTHON=1` 금지 · uv 바이너리 버전 pin. ScrapeGraphAI의 3.14 실동작만 구현 시점에 확인한다(인프라 미검증 항목).
...
- GitHub Actions를 `needs:` 체인으로 구성한다:
  `pytest·ruff·mypy 게이트` → **`ubuntu-24.04-arm` 러너에서 `docker build --platform linux/arm64`** → ECR push(`<git-sha>` + `latest`) → `aws ssm send-command` 배포 → **배포 결과 폴링 확인**.
...
- **파이썬이 3.12로 되돌아감**: 공식 이미지 내장이 3.12다. `ENV PATH="/app/.venv/bin:$PATH"` 누락 또는 `UV_SYSTEM_PYTHON=1`이 원인.
...
- [ ] **아키텍처**: 베이스 이미지·`--platform linux/arm64`·`ubuntu-24.04-arm` 러너가 전부 arm64로 정합
...
- [ ] **이미지 정합**: 베이스 = `mcr.microsoft.com/playwright/python:v1.61.0-noble`, pip `playwright==1.61.0`(이미지 태그와 일치), `uv python install 3.14` + `uv sync --python 3.14` + `ENV PATH=/app/.venv/bin:$PATH`, `pyproject.toml`의 `requires-python = ">=3.14"`, uv 바이너리는 버전 pin(`:latest` 금지), `UV_SYSTEM_PYTHON=1` 없음
