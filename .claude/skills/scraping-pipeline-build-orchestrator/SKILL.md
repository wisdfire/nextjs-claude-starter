...
- `_workspace/` 존재 여부를 확인한다.
  ...
  - **있으면**: 기존 산출물(`05_crawl_worker.md`)과 워커 코드를 읽고 이어서 진행. 수정/보완/재실행 요청이면 crawl-worker-engineer를 해당 범위로만 재호출.
...
- [ ] **브라우저**: sync API + `connect_over_cdp`, 태스크당 세션 1개, finally close, 이미지에 브라우저 바이너리 없음(python:3.14-slim)
...
- [ ] **배포**: 이미지 태그 `<agent>-<git-sha>`(latest 금지), GitOps PR 방식, 클러스터 직접 접근 없음
