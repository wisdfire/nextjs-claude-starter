...
- 웹앱의 책임은 "DB에 적재된 데이터를 읽어 렌더링"까지다.
...
- **`docs/DATA-JOBS.md`를 만들었으면** 사용자에게 **다음 행동을 명시**한다: 이 문서를 **`wisdfire/jobhub-jobs` 모노레포에 전달해 잡 구현을 요청**해야 하며, 웹앱 개발은 "DB에 적재 완료" 를 외부 의존성으로 두고 병행 가능하다. 수집 요구가 없었으면 "수집 요구 없음"이라고 명시한다.
