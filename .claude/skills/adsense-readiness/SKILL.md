...
**다국어 적용 (이 프로젝트 직결)**: 이 프로젝트는 한국어 기준 + 영어 추가 지원이다. **본문 콘텐츠를 기계번역해 영어판을 대량 생성하는 것은 이 조항 위반**이다 — 검색 스팸 정책이 "machine-translated text without human review"를 명시적 예시로 든다. 그래서 이 프로젝트의 기본 방침은 **UI 문자열만 양언어, 본문은 한국어 원문 유지**다. 본문을 번역하려면 **사람이 검토한 것만** 게시한다. (규칙 상세: `i18n-localization` 스킬)
