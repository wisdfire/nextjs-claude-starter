---
name: opentofu-infra
description: AWS EC2/보안그룹/Valkey·Redis/ECR·IAM(OIDC) 등 인프라를 OpenTofu(tofu CLI, 최신 안정 1.12.x)로 프로비저닝할 때 반드시 사용. 앱 배포가 아닌 인프라 레이어(IaC) 관리. remote state·lock·state 암호화, plan/apply 흐름, outputs를 GitHub Actions로 넘기는 연결점을 다룬다. "opentofu", "tofu", "topu", "terraform", "IaC", "인프라 프로비저닝", "EC2 생성", "보안그룹", "ECR", "remote state", "plan/apply" 요청 시 즉시 로드하라. 이 프로젝트의 IaC 표준 도구는 Terraform이 아니라 OpenTofu다.
---
...
- **OpenTofu가 담당(인프라 레이어)**: EC2 인스턴스, VPC·서브넷·보안그룹, Valkey/Redis, 컨테이너 레지스트리(ECR), IAM 역할·OIDC 공급자 같은 **오래 사는 기반 자원**의 생성·변경·삭제.
- **GitHub Actions가 계속 담당(앱 레이어)**: Docker 이미지 빌드·레지스트리 push·롤아웃 같은 **애플리케이션 배포**. 크롤링 워커 트랙에서는 arm64 이미지 빌드 → ECR push → SSM send-command 배포까지가 앱 레이어다(`celery-crawl-worker` 스킬).
...
> 크롤링 워커(MAS) 트랙에서는 실행 인프라(EC2 t4g.small · Docker Compose · Valkey · ECR · IAM/OIDC · Grafana Alloy)가 **별도 인프라 저장소 `crawling-node-infra`** 에서 이 규약으로 관리된다. 워커 저장소는 인프라를 프로비저닝하지 않는다.
>
> 그 저장소의 추가 규칙: `tofu` 명령은 `tofu/` 디렉토리 안에서만 실행하고, **`apply`·`destroy`는 사람이 plan을 읽고 승인한 뒤에만** 실행한다(`plan`에 섞인 `destroy` 한 줄이 EC2를 교체하고 EBS와 함께 **Valkey에 쌓인 크롤링 잡을 전부 날린다**). `main.tf`의 `lifecycle { ignore_changes = [ami] }`가 이 사고를 막고 있으니 제거하지 말 것.
...
- 연결 방식: `tofu output -json`으로 값을 뽑아 **GitHub Actions 시크릿/변수(`gh secret set`·`gh variable set`)로 주입**한다. 이후 앱 배포 워크플로(워커 트랙이면 ECR push·SSM 배포 job)가 이 값을 사용한다.
  - 워커 트랙이 쓰는 outputs: `ecr_repository_url`·`github_actions_role_arn`·`instance_id`. 셋 다 **비밀이 아니므로 Secrets가 아니라 Variables**에 넣는다.
- 인프라가 바뀌어 엔드포인트가 달라지면 이 outputs만 다시 넘기면 되고, 앱 배포 워크플로는 그대로다(관심사 분리의 이점).

## 5. 과설계 경고

> 단일 EC2 호스트로 충분한 규모라면 본격 K8s 클러스터·EKS 없이 **docker-compose로 충분**할 수 있다. 실제로 크롤링 워커 트랙은 K3s+KEDA 설계를 **단일 EC2 + Docker Compose로 되돌렸다** — 2 GiB 노드에서 컨트롤플레인이 잡아먹는 메모리가 정당화되지 않았기 때문이다. 트래픽·워커 수·가용성 요구가 실제로 클러스터를 정당화할 때만 프로비저닝하고, 그 전에는 **인스턴스 타입 상향(scale-up)** 을 먼저 검토하라.

## 체크리스트

- [ ] IaC 도구는 OpenTofu(`tofu` CLI)이고 `required_version`·CI로 버전이 고정된다(1.12.x)
- [ ] OpenTofu는 인프라(EC2·보안그룹·Valkey·레지스트리·IAM/OIDC)만 다루고, 앱 배포는 GitHub Actions에 남긴다
...
- [ ] 규모가 작으면 K8s 대신 docker-compose를 검토하고, 확장은 scale-up을 먼저 고려했다
