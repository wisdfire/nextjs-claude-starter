...
| 날짜 | 변경 내용 | 대상 | 사유 |
| ---- | -------- | ---- | ---- |
| 2026-07-22 | **콘텐츠 편집 뎁스(Editorial Depth)를 설계·구현 요구사항 + 적대적 감사 렌즈로 승격(streamprice 학습).** streamprice가 애드센스 적대적 감사를 `approve-with-risk`(blocker 0)로 통과하고도 콘텐츠 뎁스가 부족해 배포 후 보완 프롬프트로 개선해야 했다. **원인**: L1의 "출처 삭제 테스트"는 파생 산출물의 **0/非0**만 판정해, **존재하지만 얇거나 템플릿에 숫자만 바꿔 끼운(formulaic)** 콘텐츠("N번째로 낮습니다"가 전 페이지 동형)를 통과시킨다. 뎁스가 **마지막 게이트의 pass/fail로만** 다뤄지고 상류 설계·구현의 양의 요구사항이 아니었다. **조치(SSOT 1회 정의 + 참조)**: ①`adsense-readiness`에 **§편집 뎁스** 신설 — 뎁스=4종 편집 레이어(페이지별 해설 심화·에버그린 가이드/글·큐레이션/상호연결·방법론/FAQ/용어), 판정은 **유형 커버리지 + 비-템플릿성**으로만(⚠️ 글자수·글 수 임계값 창작 금지 원칙 유지 — 새 MUST 아님, MUST 5·6·7·10 리스크 완화) ②`adsense-adversarial-audit` L1에 **Q7 템플릿 파생 테스트**(표본 3+ 토큰 diff → 골격 동일이면 formulaic·전 사이트 단일 템플릿이면 blocker)·**Q8 뎁스 유형 커버리지 테스트** 추가 ③상류 배선: `prd-authoring`(a)·`wireframe-design`·`frontend-build`에 4종 레이어를 요구/설계/구현 규칙으로, `fullstack-web-orchestrator`(Phase 0 선언·설계/프론트 프롬프트·Phase 4.4 L1 축)·에이전트 3종(`design-architect`·`frontend-engineer`·`adsense-auditor`)에 전파 | `adsense-readiness`·`adsense-adversarial-audit`·`prd-authoring`·`wireframe-design`·`frontend-build`·`fullstack-web-orchestrator`, 에이전트 `design-architect`·`frontend-engineer`·`adsense-auditor` | streamprice 뎁스 부족(감사 통과 후 보완 프롬프트로 개선 중) 학습을 하네스에 반영 요청 |
| 2026-07-22 | **구현 완료 시 CLAUDE.md 자동 현행화 단계 신설(Phase 5).** 템플릿에서 태어난 저장소는 정체성이 제네릭(`Next.js Starter Kit`)이고 `## 이 스타터킷의 사용 흐름` 절이 빈 스타터킷 전제라, 구현을 마쳐도 다음 세션이 실제 구축 상태를 모른다(streamprice는 이 현행화를 별도 수동 요청으로 처리했음). **조치**: `fullstack-web-orchestrator` Phase 5 정리에 **CLAUDE.md 현행화** 스텝 추가 — Phase 4.3 통합 검증을 수행한 호출(단발 완주 또는 층별 마지막 층)에서만, ①정체성 헤더(제네릭이면 교체·멱등) ②`## 이 스타터킷의 사용 흐름`→`## 현재 프로젝트 상태` 절(구현 범위·데이터 모델·gotcha) ③새 명령어(db:seed 등) ④변경 이력 기록. **본문 규칙(핵심 규칙·문서 색인·하네스)은 불변**(얇은 라우터 — 프로젝트별로 갈리면 안 됨). 상위 파이프라인(`webforge-orchestrator`)의 초기 현행화(헤더+경계, Phase B-3.5)와 짝으로 완전 패리티. | `fullstack-web-orchestrator` | 상위 파이프라인에서 "streamprice처럼 초기 CLAUDE.md 현행화" 요청 — 구현후 상태 절의 소유자는 구현 하네스 |
...
| 2026-07-22 | **층별 순차 구동(scoped) 모드 신설 — P1 토큰 조치**. `fullstack-web-orchestrator`가 "프론트 구현"을 단일 태스크로 frontend-engineer에 통째로 줘, 한 세션에서 전 화면(수십~수백 파일)을 이고 가 캐시 재읽기가 준-2차로 폭발(상위 `webforge-orchestrator` 파이프라인 streamprice Phase D 실측: frontend-engineer 단독 **573M 캐시읽기** = 구현 세션 전체의 56%, 전체 캐시읽기 96%, 세션 한도 4회+ 중단). ①Phase 0에 **층별 순차 구동** 모드 분기(부분 재실행과 구분 — touch-up이 아니라 층 단위 신규 건설) ②**Phase 2-S 신설**: baseline 회수(선행 층 done 검증)·범위 한정 태스크만 등록(통합QA·AdSense·배포는 마지막 층만)·**스코프된 읽기 프롬프트**(이미 done인 층 코드 재읽기 금지 = 조치의 핵심)·커밋 후 정지 ③**`_workspace/impl-progress.md`** 인계 원장 규약 신설(층·상태·커밋·시각·다음층) ④description에 층별 트리거 키워드. 상위 오케스트레이터의 "Phase D 층별 순차 구동"과 **짝으로** 동작(각 층=새 세션이라 컨텍스트 리셋). 하위호환: 작은 ROADMAP은 기존 단발 완주 유지 | `fullstack-web-orchestrator` | 상위 파이프라인 실측에서 세션 한도 반복 중단·frontend-engineer 573M 캐시읽기 확인 후 P1 조치 요청 |
