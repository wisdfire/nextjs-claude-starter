...
**웹앱이 수집 데이터를 소비하는 경계**만 이 저장소의 범위다:

- 수집 결과는 **Supabase(PostgreSQL)에 이미 적재된 상태**로 도착한다고 가정하고, 웹앱은 이를 **읽어서 렌더링**한다.
- 웹앱에서 필요한 것은 **읽기 쿼리·타입·캐싱 전략**뿐이다 (`docs/guides/architecture.md`의 Supabase 클라이언트 규약 참고).
