...
- **언어·런타임**: Python **3.14** — 인프라 지정. ★ 베이스가 `resolute`(Ubuntu 26.04)라 이미지 내장 **시스템 Python이 이미 3.14**다. 그래도 Dockerfile에서 `uv python install 3.14 && uv sync --frozen --no-dev --python 3.14` 후 `ENV PATH="/app/.venv/bin:$PATH"`로 uv 관리 버전(3.14.6 pin)을 앞에 두는 이유는, 3.12 회피가 아니라 **패치를 uv.lock에 고정해 재현성을 확보**(시스템 Python은 이미지 재빌드마다 패치 드리프트)하기 위해서다. **`UV_SYSTEM_PYTHON=1`을 걸면 안 된다**(시스템 3.14로 되돌아가 패치 고정이 풀린다). `pyproject.toml`의 `requires-python = ">=3.14"`와 어긋나면 `uv sync`가 거부한다. ⚠️ **Celery 5.6.3은 3.14를 공식 분류자에 선언하지 않았다**(인프라가 arm64에서 Python 3.14.6·uv 0.11.28·Celery 5.6.3·Chromium 149로 실동작 검증했으나 공식 보증은 없음) — prefork 스모크 테스트를 CI 게이트에 필수로 두고, 깨지면 3.13으로 내린다
