...
4. **애드센스 게이트 (필수 — 사용자에게 보이는 화면·콘텐츠가 바뀐 경우)**
   ...
   > **위 목록은 기계적으로 확인 가능한 것까지다.** 콘텐츠 가치·복제 여부·정책 페이지의 실질 내용은 Grep으로 판정되지 않는다. 화면·콘텐츠가 크게 바뀌었거나 배포 직전이라면 **`adsense-adversarial-audit` 스킬의 적대적 검증**(심사관 3렌즈 + 최대 3라운드 수정·재심사)을 수행한다. **존재는 충족이 아니다** — `app/privacy/page.tsx`가 있어도 내용이 껍데기면 위 체크리스트는 통과하고 심사는 떨어진다.
