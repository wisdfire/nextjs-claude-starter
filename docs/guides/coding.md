...
- **비밀·키 하드코딩 금지 / 서버 값 클라이언트 노출 금지**: Supabase 서비스 키·`DATABASE_URL` 같은 비밀을 코드에 직접 쓰지 않는다(환경 변수로만). 클라이언트에 노출돼선 안 되는 값에 **`NEXT_PUBLIC_` 접두사를 붙이지 않는다**. 서버 전용 값(Drizzle `db`·서버 Supabase 클라이언트 등)이 클라이언트 번들에 섞이면 치명적이다.
