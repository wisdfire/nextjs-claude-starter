...
| 백엔드 · 데이터 | **Neon Postgres** — 서버리스 Postgres (scale-to-zero). 모든 접근은 서버사이드 단일 경로                     |
| ORM · DB        | **Drizzle ORM** (`drizzle-orm` · `postgres` · `drizzle-kit`) — Neon Postgres에 타입 안전 쿼리/마이그레이션  |
| 국제화          | **next-intl v4** — 배선 완료. **한국어 기준 + 영어 추가 지원** · 언어 전환 컴포넌트 · **en.json은 커밋 훅이 LLM으로 자동 번역** (상세는 tech-stack 가이드) |
...
cp .env.example .env.local   # Neon 연결 문자열 입력 (DATABASE_URL · DIRECT_URL)
npm run db:push              # 스키마를 DB에 반영 (로컬 프로토타이핑 전용)
...
lib/
  ...
  db/                 # Drizzle ORM (index 서버 전용 클라이언트 · schema)
drizzle.config.ts     # drizzle-kit 설정 (DIRECT_URL 사용)
tests/                # Vitest 전역 셋업
...
- [`docs/guides/db-operations.md`](docs/guides/db-operations.md) — 배포 시 스키마 반영 절차 (마이그레이션)
